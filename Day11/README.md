...
How to parse multi-line structures like:
...
- a new line separates monkey features: starting items, operation, test, throw
- an empty line separates monkeys

So, parse each feature in order?

Parsing the operation which has the form:

- `old op decimal` if op is `+`
OR
- `old * old` if op is `*` which is the square function
...
Monkey structure:
  ...
  operation : (Int -> Int)
  test : Int
  throw : (Bool -> Int)  -- f mod (test) == 0 (is divisible) then n else m
  items : Map where key is item : int, and value is count : Int
```

```haskell
instance Show Monkey where
  show (Monkey i is op) = "id: " ++ show i
                          ++ ", items: " ++ show is
                          ++ ", op: " ++ show op

instance Eq Monkey where
  Monkey m i o == Monkey m' i' o' = (m == m')     -- ids match
                                    && (i == i')  -- items match
                                    && (o == o')  -- operations match
```
